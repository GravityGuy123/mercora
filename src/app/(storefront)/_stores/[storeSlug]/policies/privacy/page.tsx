...
type StorefrontPrivacyPolicyPageProps = {
  params: Promise<{
    storeSlug: string;
  }>;
};
...
export async function generateMetadata({
  params,
}: StorefrontPrivacyPolicyPageProps): Promise<Metadata> {
  const { storeSlug } = await params;
  const storefront = await getPublicStorefrontBySlug(storeSlug);

  if (!storefront) {
    return {
      title: "Privacy Policy | Storefront Not Found",
    };
  }

  return {
    title: `Privacy Policy | ${storefront.store_name}`,
    description: `Privacy policy for ${storefront.store_name}.`,
  };
}

export default async function StorefrontPrivacyPolicyPage({
  params,
}: StorefrontPrivacyPolicyPageProps) {
  const { storeSlug } = await params;
  const storefront = await getPublicStorefrontBySlug(storeSlug);
  ...
  return (
    <main className="bg-[#040A18] text-white">
      <PolicyShell
        ...
        description={`Privacy and data handling information for ${storefront.store_name}.`}
        content={storefront.privacy_policy}
        backHref={basePath}
      />
    </main>
  );
