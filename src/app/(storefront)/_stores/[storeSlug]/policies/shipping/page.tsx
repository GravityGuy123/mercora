...
type StorefrontShippingPolicyPageProps = {
  params: Promise<{
    storeSlug: string;
  }>;
};
...
export async function generateMetadata({
  params,
}: StorefrontShippingPolicyPageProps): Promise<Metadata> {
  const { storeSlug } = await params;
  const storefront = await getPublicStorefrontBySlug(storeSlug);

  if (!storefront) {
    return {
      title: "Fulfillment Policy | Storefront Not Found",
    };
  }

  return {
    title: `Fulfillment Policy | ${storefront.store_name}`,
    description: `Fulfillment policy for ${storefront.store_name}.`,
  };
}

export default async function StorefrontShippingPolicyPage({
  params,
}: StorefrontShippingPolicyPageProps) {
  const { storeSlug } = await params;
  const storefront = await getPublicStorefrontBySlug(storeSlug);
  ...
  return (
    <main className="bg-[#040A18] text-white">
      <PolicyShell
        ...
        content={storefront.fulfillment_policy}
        backHref={basePath}
      />
    </main>
  );
